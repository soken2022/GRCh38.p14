...
def portion_of_TE_elements_in_geneme_with_UCSC_and_refseq(sequence0, gff,chnumber):
...
def nucleotide_composition_in_seqenence_simple(sequence):
...
def nucleotide_composition_in_seqenence_ATCG_2(sequence):
    ...
    print('Element_type1_portion',one_masked_portion_type1,'Element_type2_portion',one_masked_portion_type2,'%','Both portion',Duble_masked_portion,'%','unmasked',unmasked_region,'%')
                                                                                                                 #'+unsequenced',unsequenced_region,'%')
    ...
    print('\nnucleotide_composition_in_seqenence Pass!!!!')
    # Sample data
    labels = ['GFF', 'TEs','Shered-GFF-TEs', 'unmasked_region']#, 'un_sequenced']
    ...
    return one_masked_portion_type1,one_masked_portion_type2,Duble_masked_portion,unmasked_region,unsequenced_region
...
def analysis_composition_of_labled_sequence(sequence_plus,sequence_comp,sequence_both):
    un_sequenced_length = sequence_plus.count('N')  + sequence_plus.count('n')
    sequenced_length = len(sequence_plus) - un_sequenced_length
    unmasked_portion = (sequence_both.count('a') + sequence_both.count('t') + sequence_both.count('c') + sequence_both.count('g'))/sequenced_length * 100
    plus_portion = (sequence_plus.count('A') + sequence_plus.count('T') + sequence_plus.count('C') + sequence_plus.count('G'))/sequenced_length * 100
    minis_portion = (sequence_comp.count('A') + sequence_comp.count('T') + sequence_comp.count('C') + sequence_comp.count('G'))/sequenced_length*100
    both_portion = (sequence_both.count('A')+sequence_both.count('T')+sequence_both.count('C')+sequence_both.count('G'))/sequenced_length * 100
    shered_portion = plus_portion+minis_portion - both_portion
    print('sequenced portion',round(sequenced_length/len(sequence_plus)*100,2))
    print('Plus portion',plus_portion-shered_portion)
    print('Negative portion',minis_portion-shered_portion)
    print('shered portion',(plus_portion+minis_portion)-both_portion)
    print('Both Plus and Negative portion and shered',both_portion)
    print('unmasked portion',unmasked_portion)
    print('calculat_portion_of_elements Pass!!')
    labels = ['+ usage', '- usage','Shered', 'unmasked_region']#, 'un_sequenced']
    sizes = [plus_portion-shered_portion,minis_portion-shered_portion,(plus_portion+minis_portion)-both_portion,unmasked_portion]#,unsequenced_region]  # percentages, should add up to 100%
    #print('sizes',sizes)
    explode = (0.1, 0.1, 0.1, 0.1)
    ...
    # Adding a title
    plt.title('+ and - composition')

    # Display the plot

    plt.show()
    P = plus_portion-shered_portion
    C = minis_portion-shered_portion
    Sh = (plus_portion+minis_portion)-both_portion
    U = unmasked_portion
    return    P,C,Sh,U
...
def merge_seq_seq_share_with_1234(seq1,seq2):
    print('merging GFF and TEs information')
    C = []
...
def merge_plus_with_plus_and_comp_with_comp_share_with_1234(seq1,seq2):
    print('merging GFF and TEs information_plus_with_plus_and_comp_with_comp')
    C = []
    for i,j in zip(seq1,seq2):
        Ni = False
        Nj = False
...
def only_ATCG_and_atcg(sequence):
    print('convert multi labling to only ATCG and atcg')
    M_sequence = ''.join(sequence)
    M_sequence = M_sequence.replace('~','A')
    M_sequence = M_sequence.replace('!','T')
    M_sequence = M_sequence.replace('@','C')
    M_sequence = M_sequence.replace('#','G')
    M_sequence = M_sequence.replace('1','A')
    M_sequence = M_sequence.replace('2','T')
    M_sequence = M_sequence.replace('3','C')
    M_sequence = M_sequence.replace('4','G')
...
def portion_of_TE_elements_in_geneme_with_UCSC_and_refseq_step2(sequence0, out,ch_number,element):
    sequence = sequence0.lower()  # ATCG all
    sequence_plus = sequence
    sequence_plus = list(sequence_plus) #ATCG all
    sequence_comp = sequence
    ...
    for annotation in gff:
        ...
        #print(ch_number,element)
        #print(annotation[4],annotation[10])
        if (f'chr{ch_number[0]}' == annotation[4] or ch_number[1] == annotation[4])  and element in annotation[10]:
...
def exon_and_intron_labaling(sequence0, gff,chnumber):
    link = gff
    sequence = sequence0.lower()  # ATCG all
    sequence_plus = sequence
    sequence_plus = list(sequence_plus) #ATCG all
    ...
    sequence_both = sequence
    sequence_both = list(sequence_both) #ATCG all
    sequence_upper = sequence.upper() # this is for masking
    ...
    for annotation in gff:
        ...
        if (f'chr{chromosome_number[0]}' == annotation[0] or chromosome_number[1] in annotation[0]) and ('gene' in annotation[2] ):
            counter2 += 1
            start = int(annotation[3])
            end = int(annotation[4])
            if end - start == 0:
            ...
            if annotation[6] == '+':
                sequence_plus[(start - 1):end] = sequence_upper[(start - 1):end]
                sequence_both[(start - 1):end] = sequence_upper[(start - 1):end]
                #print('sample',len(sequence0[(start - 1):end]),sequence0[(start - 1):end])


            if annotation[6] == '-':
                sequence_comp[start - 1:end] = sequence_upper[start - 1:end]
                sequence_both[(start - 1):end] = sequence_upper[(start - 1):end]
    gff.close()
    gff.close()
    ...
    for annotation in gff:
        ...
        if (f'chr{chromosome_number[0]}' == annotation[0] or chromosome_number[1] in annotation[0]) and ('exon' in annotation[2] ):
            counter2 += 1
            start = int(annotation[3])
            end = int(annotation[4])
            if end - start == 0:
            ...
            if annotation[6] == '+':
                exon_sequence = sequence_plus[(start - 1):end]
                exon_sequence = ''.join(exon_sequence)
                exon_sequence = exon_sequence.replace('A','1')
                exon_sequence = exon_sequence.replace('T','2')
                exon_sequence = exon_sequence.replace('C','3')
                exon_sequence = exon_sequence.replace('G','4')
                exon_sequence = exon_sequence.replace('a','5')
                exon_sequence = exon_sequence.replace('t','6')
                exon_sequence = exon_sequence.replace('c','7')
                exon_sequence = exon_sequence.replace('g','8')
                sequence_plus[(start - 1):end] = exon_sequence

                exon_sequence = sequence_both[(start - 1):end]
                exon_sequence = ''.join(exon_sequence)
                exon_sequence = exon_sequence.replace('A','1')
                exon_sequence = exon_sequence.replace('T','2')
                exon_sequence = exon_sequence.replace('C','3')
                ...
                sequence_both[(start - 1):end] = exon_sequence
                #print('sample',len(sequence0[(start - 1):end]),sequence0[(start - 1):end])

            if annotation[6] == '-':
                exon_sequence = sequence_comp[(start - 1):end]
                exon_sequence = ''.join(exon_sequence)
                exon_sequence = exon_sequence.replace('A','1')
                exon_sequence = exon_sequence.replace('T','2')
                exon_sequence = exon_sequence.replace('C','3')
                ...
                exon_sequence = exon_sequence.replace('c','7')
                exon_sequence = exon_sequence.replace('g','8')
                sequence_comp[(start - 1):end] = exon_sequence
    ...
    entron_P = round((sequence_plus.count('A') + sequence_plus.count('T') + sequence_plus.count('C') + sequence_plus.count('G'))/sequenced_length*100,2)
    exon_p = round((sequence_plus.count('1') + sequence_plus.count('2') + sequence_plus.count('3') + sequence_plus.count('4'))/sequenced_length*100,2)
    onlhy_exon = round((sequence_plus.count('5') + sequence_plus.count('6') + sequence_plus.count('7') + sequence_plus.count('8'))/sequenced_length*100,2)
    print('plus_information','entron',entron_P,'exon',exon_p,'onlhy exon',onlhy_exon)

    entron_C = round((sequence_comp.count('A') + sequence_comp.count('T') + sequence_comp.count('C') + sequence_comp.count('G'))/sequenced_length*100,2)
    exon_C = round((sequence_comp.count('1') + sequence_comp.count('2') + sequence_comp.count('3') + sequence_comp.count('4'))/sequenced_length*100,2)
    onlhy_exon_C = round((sequence_comp.count('5') + sequence_comp.count('6') + sequence_comp.count('7') + sequence_comp.count('8'))/sequenced_length*100,2)
    print('comp_information','entron',entron_C,'exon',exon_C,'onlhy exon',onlhy_exon_C)

    entron_B = round((sequence_both.count('A') + sequence_both.count('T') + sequence_both.count('C') + sequence_both.count('G'))/sequenced_length*100,2)
    exon_B = round((sequence_both.count('1') + sequence_both.count('2') + sequence_both.count('3') + sequence_both.count('4'))/sequenced_length*100,2)
    onlhy_exon_B = round((sequence_both.count('5') + sequence_both.count('6') + sequence_both.count('7') + sequence_both.count('8'))/sequenced_length*100,2)
    print('Both_information','entron',entron_B,'exon',exon_B,'onlhy exon',onlhy_exon_B)
...
def d2024_11_18_plus_and_minis_lableing_gff_out(sequence0, gff,chnumber):
    link = gff
    sequence = sequence0.upper()  # ATCG all
    ...
    for annotation in gff:
        ...
        if (f'chr{chromosome_number[0]}' == annotation[0] or chromosome_number[1] in annotation[0] and annotation[2]=='gene') :
            counter2 += 1
            start = int(annotation[3])
            end = int(annotation[4])
            if end - start == 0:
            ...
            if annotation[6] == '+':
                exon_sequence = sequence[(start - 1):end]
                exon_sequence = ''.join(exon_sequence)
                exon_sequence = exon_sequence.replace('1','A')
                exon_sequence = exon_sequence.replace('2','T')
                exon_sequence = exon_sequence.replace('3','C')
                exon_sequence = exon_sequence.replace('4','G')
                exon_sequence = exon_sequence.replace('a','5')
                exon_sequence = exon_sequence.replace('t','6')
                exon_sequence = exon_sequence.replace('c','7')
                exon_sequence = exon_sequence.replace('g','8')
                sequence[(start - 1):end] = exon_sequence
    ...
    print((sequence.count('A')+sequence.count('T')+sequence.count('C')+sequence.count('G'))/sequenced_length*100)
    print((sequence.count('a')+sequence.count('t')+sequence.count('c')+sequence.count('g'))/sequenced_length*100)
    print((sequence.count('5')+sequence.count('6')+sequence.count('7')+sequence.count('8'))/sequenced_length*100)
